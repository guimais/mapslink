(() => {
  ...
  let overlay = null, map, markersLayer, markerById = new Map(), heatLayer = null, currentFilters = {}, currentList = [];
  ...
  function openDrawer() {
    if (!painelFiltros) return;
    if (mqDrawer.matches) painelFiltros.hidden = true; else painelFiltros.hidden = false;
  ...
  function popupHtml(c) {
    ...
    const jobs = (Array.isArray(c.jobs) ? c.jobs : []).map(j => `<li><a href="${j.url}" target="_blank" rel="noopener">${j.title} (${j.type})</a></li>`).join("");
    const hiring = c.is_hiring ? `<span style="padding:.2rem .5rem;border-radius:9999px;background:#16a34a;color:#fff;font-size:.75rem;">Contratando</span>` : `<span style="padding:.2rem .5rem;border-radius:9999px;background:#94a3b8;color:#fff;font-size:.75rem;">Sem vagas</span>`;
  ...
  function renderHeat(list, on) {
    ...
    heatLayer = L.heatLayer(points, { radius: 36, blur: 24, maxZoom: 17, minOpacity: 0.25, max: Math.max(20, Math.max(1, points.length) * 3) });
    heatLayer.addTo(map);
  }
  function applyAndRender(filters) {
    currentFilters = { ...currentFilters, ...(filters || {}) };
    const result = MapsFilters.filterCompanies(window.__companies, currentFilters);
    currentList = result;
    clearMarkers();
    addMarkers(result);
    renderList(result);
  ...
  (async () => {
    ...
    try {
      ...
      companies = Array.isArray(window.__companies) ? window.__companies : [];
    }
    window.__companies = companies;
    map = L.map(mapEl, { scrollWheelZoom: true });
    L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", { maxZoom: 19, attribution: "&copy; OpenStreetMap" }).addTo(map);
    markersLayer = L.layerGroup().addTo(map);
    currentList = MapsFilters.filterCompanies(window.__companies, {});
    ...
    window.getFilterOptions = function () { return MapsFilters.buildFilterOptions(window.__companies); };
