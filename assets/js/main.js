(() => {
  ...
  const siteRoot = scriptUrl ? new URL("../../", scriptUrl).href : window.location.origin + "/";
  ...
  const usersUrl = resolvePath("assets/data/users.json");
  ...
  const JOBS_PREFIX = "mapslink:jobs";
  const PUBLIC_KEY = "mapslink:jobs:public";
  const EVENT_NAME = "mapslink:jobs:update";
  ...
  window.loadCompanies = async function loadCompanies() {
    try {
      const url = window.MapsUtils?.resolvePath
        ? window.MapsUtils.resolvePath("assets/data/companies.json")
        : "assets/data/companies.json";
      const response = await fetch(url, { cache: "no-store" });
      if (!response.ok) throw new Error("Failed to load companies");
      return await response.json();
    } catch (error) {
      console.error("MapsLink: Error loading companies", error);
      return [];
    }
  };
  ...
  window.loadUsers = async function loadUsers() {
    try {
      const url = window.MapsUtils?.resolvePath
        ? window.MapsUtils.resolvePath("assets/data/users.json")
        : "assets/data/users.json";
      const response = await fetch(url, { cache: "no-store" });
      if (!response.ok) throw new Error("Failed to load users");
      return await response.json();
    } catch (error) {
      console.error("MapsLink: Error loading users", error);
      return [];
    }
  };
