(function () {
  ...
  const getNavLinks = () => {
    if (window.MapsApp && typeof window.MapsApp.navLinks === "function") {
      try { return window.MapsApp.navLinks() || []; } catch {}
    }
  ...
  // ---- Destaque do menu conforme seção em viewport ----
  const navObserver = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      ...
      if (window.MapsApp && typeof window.MapsApp.highlightNav === "function") {
        try { window.MapsApp.highlightNav(rawId); return; } catch {}
      }

      const link = linkByKey.get(`#${idKey}`) || linkByKey.get(idKey);
      if (!link) return;
