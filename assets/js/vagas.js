(function(){
  ...
  function aplicarFiltros(){
    ...
    if(btnFiltrar){
      btnFiltrar.classList.toggle('active', ativo);
    }
    try{ localStorage.setItem('cvFilters', JSON.stringify(f)); }catch{}
  }
  ...
  diInput && diInput.addEventListener('input', ()=> maskDateBRInput(diInput));
  dfInput && dfInput.addEventListener('input', ()=> maskDateBRInput(dfInput));
  ...
  document.addEventListener('click', (e)=>{
    if(!painel || painel.hidden) return;
    if(!painel.contains(e.target) && e.target !== btnFiltrar){
      fecharPainel();
    }
  });
  document.addEventListener('keydown', (e)=>{ if(e.key === 'Escape') fecharPainel(); });

  form?.addEventListener('submit', (e)=>{ e.preventDefault(); aplicarFiltros(); fecharPainel(); });
...
(function () {
  ...
  const campoDescricao = document.getElementById('vaga-descricao'); // guardado no estado p/ uso futuro
  ...
  /** @type {{id:string,titulo:string,area:string,tipo:string,descricao?:string,dataPub:string,status:'Aberta'|'Fechada'}[]} */
  let vagas = [];
  ...
  function save() {
    try { localStorage.setItem(LS_KEY, JSON.stringify(vagas)); } catch {}
  }
