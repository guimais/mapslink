(() => {
  ...
  const fields = {
    ...
    businessCategory: { input: "#businessCategory", error: "#businessCategoryError" },
    otherCategory: { input: "#otherCategory", error: "#otherCategoryError" },
    responsibleName: { input: "#responsibleName", error: "#responsibleNameError" },
    responsiblePosition: { input: "#responsiblePosition", error: "#responsiblePositionError" },
    responsibleEmail: { input: "#responsibleEmail", error: "#responsibleEmailError" },
    responsiblePhone: { input: "#responsiblePhone", error: "#responsiblePhoneError" },
  ...
  function formatCnpj(input) {
    ...
    if (value.length >= 14) value = value.replace(/(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, "$1.$2.$3/$4-$5");
    else if (value.length >= 12) value = value.replace(/(\d{2})(\d{3})(\d{3})(\d{4})(\d{0,2})/, "$1.$2.$3/$4-$5");
    else if (value.length >= 9) value = value.replace(/(\d{2})(\d{3})(\d{3})(\d{0,4})/, "$1.$2.$3/$4");
    else if (value.length >= 6) value = value.replace(/(\d{2})(\d{3})(\d{0,3})/, "$1.$2.$3");
    else if (value.length >= 3) value = value.replace(/(\d{2})(\d{0,3})/, "$1.$2");
    input.value = value;
  }
  ...
  function validateCompanyName() {
    ...
    if (value.length < 2) return setError("companyName", "Nome da empresa deve ter pelo menos 2 caracteres");
  ...
  function validateCompanyPhone() {
    ...
    if (!value) return setError("companyPhone", "Telefone da empresa é obrigatório");
    if (value.length < 10) return setError("companyPhone", "Telefone deve ter pelo menos 10 dígitos");
    if (value.length > 11) return setError("companyPhone", "Telefone deve ter no máximo 11 dígitos");
    return clearError("companyPhone");
  }

  function validateCompanyEmail() {
    const value = state.elements.companyEmail?.value.trim() || "";
    if (!value) return setError("companyEmail", "E-mail da empresa é obrigatório");
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) return setError("companyEmail", "Informe um e-mail válido");
  ...
  function toggleOtherCategory() {
    ...
    if (selected === "outros") {
      ...
      setTimeout(() => { state.otherGroup.style.display = "none"; }, 300);
  ...
  function validateBusinessCategory() {
    ...
    if (!value) return setError("businessCategory", "Categoria de negócio é obrigatória");
  ...
  function validateOtherCategory() {
    ...
    if (value.length < 2) return setError("otherCategory", "Categoria deve ter pelo menos 2 caracteres");
    return clearError("otherCategory");
  }

  function validateResponsibleName() {
    const value = state.elements.responsibleName?.value.trim() || "";
    if (!value) return setError("responsibleName", "Nome do responsável é obrigatório");
    if (value.length < 2) return setError("responsibleName", "Nome deve ter pelo menos 2 caracteres");
    if (!/^[a-zA-ZÀ-ÿ\s]+$/.test(value)) return setError("responsibleName", "Nome deve conter apenas letras");
  ...
  function validateResponsiblePosition() {
    ...
    if (value.length < 2) return setError("responsiblePosition", "Cargo deve ter pelo menos 2 caracteres");
  ...
  function validateResponsibleEmail() {
    ...
    if (!value) return setError("responsibleEmail", "E-mail do responsável é obrigatório");
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) return setError("responsibleEmail", "Informe um e-mail válido");
  ...
  function validateResponsiblePhone() {
    ...
    if (!value) return setError("responsiblePhone", "Telefone do responsável é obrigatório");
    if (value.length < 10) return setError("responsiblePhone", "Telefone deve ter pelo menos 10 dígitos");
    if (value.length > 11) return setError("responsiblePhone", "Telefone deve ter no máximo 11 dígitos");
  ...
  function buttonState(stateName, message) {
    ...
    if (stateName === "loading") {
      state.button.innerHTML = '<div class="loading-spinner"></div><span class="button-text">Criando conta empresarial...</span>';
  ...
  function resetButton() {
    state.button.innerHTML = '<span class="button-text">Criar Conta Empresarial</span><i class="ri-arrow-right-line button-icon"></i>';
  ...
  function createPayload() {
    const data = {
      ...
      responsiblePosition: state.elements.responsiblePosition?.value.trim() || "",
    ...
    const selectedOption = state.elements.businessCategory?.selectedOptions?.[0];
    const categoryLabel = data.businessCategory === "outros"
      ? data.otherCategory
    ...
    return {
      ...
      profile: {
        ...
        contact: {
          instagram: "@" + (data.companyName || "").replace(/\s+/g, "").toLowerCase(),
  ...
  async function submit(event) {
    ...
    try {
      ...
      setTimeout(() => { window.location.href = "loginempresa.html"; }, 1800);
    } catch (error) {
      const message =
        error?.message === "EMAIL_TAKEN" ? "Este e-mail já está em uso" :
        error?.message === "CNPJ_TAKEN" ? "Este CNPJ já está em uso" :
        ...
        (error?.message || "Erro ao criar conta empresarial. Tente novamente.");
      buttonState("error");
      alert(message);
    }
  }
  ...
  function bindFieldEvents() {
    ...
    if (state.elements.cnpj) state.elements.cnpj.addEventListener("input", () => { formatCnpj(state.elements.cnpj); validateCnpj(); });
    if (state.elements.companyPhone) state.elements.companyPhone.addEventListener("input", () => { formatPhone(state.elements.companyPhone); validateCompanyPhone(); });
    state.elements.companyEmail?.addEventListener("input", validateCompanyEmail);
    ...
    state.elements.otherCategory?.addEventListener("input", validateOtherCategory);
    state.elements.responsibleName?.addEventListener("input", validateResponsibleName);
    state.elements.responsiblePosition?.addEventListener("input", validateResponsiblePosition);
    state.elements.responsibleEmail?.addEventListener("input", validateResponsibleEmail);
    if (state.elements.responsiblePhone) state.elements.responsiblePhone.addEventListener("input", () => { formatPhone(state.elements.responsiblePhone); validateResponsiblePhone(); });
    if (state.elements.password) state.elements.password.addEventListener("input", () => { validatePassword(); checkPasswordStrength(); });
  ...
  function setupMobileExperience() {
    ...
    if ("ontouchstart" in window) {
      const touchables = document.querySelectorAll(".register-button, .password-toggle, .checkbox-container, .personal-register-link, .login-link");
