(() => {
  ...
  function collectWorkModes(company) {
    return toArray(company?.work_modes ?? company?.workModes).map((value) =>
      normalizeText(value || ""),
    );
  }
  ...
  function filterCompanies(list = [], filters = {}) {
    ...
    const industries = toArray(
      filters.industries || filters.industry || filters.sector,
    )
      .map((value) => normalizeText(value || ""))
      .filter(Boolean);
    const sizes = toArray(filters.sizes || filters.size || filters.companySize)
      .map((value) => normalizeText(value || ""))
      .filter(Boolean);
    const workModes = toArray(
      filters.workModes || filters.workMode || filters.modalities,
    )
      .map((value) => normalizeText(value || ""))
      .filter(Boolean);
    ...
    return list.filter((company) => {
      ...
      const companySize = normalizeText(
        company?.size ?? company?.company_size ?? "",
      );
      ...
      if (industries.length && !industries.includes(companyIndustry))
        return false;
      if (sizes.length && !sizes.includes(companySize)) return false;
      if (
        workModes.length &&
        !workModes.every((mode) => companyModes.includes(mode))
      )
        return false;

      if (wantsHiring != null && Boolean(company?.is_hiring) !== wantsHiring)
        return false;
      if (tags.length && !tags.every((tag) => companyTags.includes(tag)))
        return false;
      if (query && !normalizeText(collectBlob(company)).includes(query))
        return false;
