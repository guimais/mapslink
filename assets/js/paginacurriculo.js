(function () {
  ...
  function init() {
    ...
    function parseDateFromRow(row) {
      ...
      if (time?.getAttribute("datetime")) return new Date(time.getAttribute("datetime"));
    ...
    function positionPanel() {
      if (!painel || painel.hidden || !btnFiltrar) return;
      ...
      left = Math.max(
        16 + window.scrollX,
        Math.min(left, window.scrollX + window.innerWidth - panelWidth - 16)
      );
    ...
    try {
      ...
      if (saved) {
        ...
        if (fs("f-status")) fs("f-status").value = saved.status || "";
        if (fs("f-vaga")) fs("f-vaga").value = saved.vaga || "";
        if (fs("f-data-inicio")) fs("f-data-inicio").value = saved.di || "";
        if (fs("f-data-fim")) fs("f-data-fim").value = saved.df || "";
        if (buscaInput) buscaInput.value = saved.busca || "";
      }
    } catch {}
    aplicarFiltros(); // aplica sempre (com ou sem saved)
  }
